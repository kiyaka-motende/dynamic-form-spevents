...
import { Checkbox, ChoiceGroup, css, FocusZone, FocusZoneDirection, FontIcon, getRTLSafeKeyCode, GroupedList, GroupHeader, KeyCodes, Label, Link, Spinner } from 'office-ui-fabric-react';
...
export function TaxonomyTree(props) {
    ...
    var updateTaxonomyTreeViewWithNewTermItems = function (newTermItems) {
        ...
        var _loop_1 = function (term) {
            ...
            groupToAddTermTo.children = __spreadArrays((_b = groupToAddTermTo.children) !== null && _b !== void 0 ? _b : [], [g]);
            props.setTerms(function (prevTerms) {
                var nonExistingTerms = newTermItems.filter(function (newTerm) { return prevTerms.every(function (prevTerm) { return prevTerm.id !== newTerm.id; }); });
    ...
    var updateTaxonomyTreeViewWithUpdatedTermItems = function (updatedTermItems) {
        ...
        var _loop_2 = function (term) {
            ...
            props.setTerms(function (prevTerms) {
                return __spreadArrays(prevTerms.filter(function (t) { return t.id !== term.id; }), [term]);
            });
        };
    ...
    var updateTaxonomyTreeViewWithDeletedTermItems = function (deletedTermItems) {
        var _loop_3 = function (term) {
            ...
            props.setTerms(function (prevTerms) {
                return __spreadArrays(prevTerms.filter(function (t) { return t.id !== term.id; }));
            });
        };
    ...
    React.useEffect(function () {
        ...
        if (props.termSetInfo.childrenCount > 0) {
            props.onLoadMoreData(Guid.parse(props.termSetInfo.id), props.anchorTermInfo ? Guid.parse(props.anchorTermInfo.id) : Guid.empty, '', props.hideDeprecatedTerms)
                ...
                props.setTerms(function (prevTerms) {
                    var nonExistingTerms = loadedTerms.value.filter(function (newTerm) { return prevTerms.every(function (prevTerm) { return prevTerm.id !== newTerm.id; }); });
                    return __spreadArrays(prevTerms, nonExistingTerms);
                });
                rootGroup.children = grps;
                rootGroup.data.skiptoken = loadedTerms.skiptoken;
                rootGroup.hasMoreData = loadedTerms.skiptoken !== '';
                setGroupsLoading(function (prevGroupsLoading) { return prevGroupsLoading.filter(function (value) { return value !== props.termSetInfo.id; }); });
                setGroups([rootGroup]);
            });
        }
    }, []);
    ...
    var onRenderTitle = function (groupHeaderProps) {
        ...
        var isChildSelected = function (children) {
            var aChildIsSelected = children && children.some(function (child) { return props.selection && props.selection.isKeySelected(child.key) || isChildSelected(child.children); });
        ...
        if (groupHeaderProps.group.level === 0) {
            var labelStyles = { root: { width: "100%", fontWeight: childIsSelected ? "bold" : "normal" } };
            return (React.createElement(FocusZone, { direction: FocusZoneDirection.horizontal, className: styles.taxonomyItemFocusZone },
                ...
                React.createElement("div", { className: styles.actionButtonContainer }, props.onRenderActionButton && props.onRenderActionButton(props.termStoreInfo, props.termSetInfo, props.anchorTermInfo, updateTaxonomyTreeView))));
        }
        if (!props.selection) {
            var labelStyles = { root: { width: "100%", fontWeight: childIsSelected ? "bold" : "normal" } };
            var taxonomyItemIconName = groupHeaderProps.group.data.term.isDeprecated ? "Blocked" : "Tag";
            return (React.createElement(FocusZone, { direction: FocusZoneDirection.horizontal, className: styles.taxonomyItemFocusZone },
                props.showIcons && (React.createElement(FontIcon, { iconName: taxonomyItemIconName, className: styles.taxonomyItemIcon })),
                React.createElement(Label, { styles: labelStyles }, groupHeaderProps.group.name),
                React.createElement("div", { className: styles.actionButtonContainer }, props.onRenderActionButton && props.onRenderActionButton(props.termStoreInfo, props.termSetInfo, groupHeaderProps.group.data.term, updateTaxonomyTreeView))));
        }
        ...
        var isSelected = props.selection && props.selection.isKeySelected(groupHeaderProps.group.key);
        if (props.allowMultipleSelections) {
            ...
            var choiceGroupOptionStyles = isSelected || childIsSelected ? { root: { marginTop: 0 }, choiceFieldWrapper: { fontWeight: 'bold', flex: '1' }, field: { width: '100%' } } : { root: { marginTop: 0 }, choiceFieldWrapper: { fontWeight: 'normal', flex: '1' }, field: { width: '100%' } };
            var options = [{
            ...
            var choiceGroupStyles = { root: { flex: "1" }, applicationRole: { width: "100%" } };
            return (React.createElement(FocusZone, { direction: FocusZoneDirection.horizontal, className: styles.taxonomyItemFocusZone },
    ...
    var onRenderHeader = function (headerProps) {
        ...
        var isDisabled = headerProps.group.data.term && headerProps.group.data.term.isAvailableForTagging.filter(function (t) { return t.setId === props.termSetInfo.id; })[0].isAvailable === false;
        return (React.createElement(GroupHeader, __assign({}, headerProps, { styles: groupHeaderStyles, className: styles.taxonomyItemHeader, onRenderTitle: onRenderTitle, onToggleCollapse: onToggleCollapse, indentWidth: 20, expandButtonProps: { style: { color: (_a = props.themeVariant) === null || _a === void 0 ? void 0 : _a.semanticColors.bodyText } }, onGroupHeaderKeyUp: function (ev, group) {
            ...
            return (React.createElement("div", { className: styles.loadMoreContainer },
                React.createElement(Link, { onClick: function () {
                        ...
                        props.onLoadMoreData(Guid.parse(props.termSetInfo.id), footerProps.group.key === props.termSetInfo.id ? Guid.empty : Guid.parse(footerProps.group.key), footerProps.group.data.skiptoken, props.hideDeprecatedTerms)
                            ...
                            var nonExistingChildren = grps.filter(function (grp) { var _a; return (_a = footerProps.group.children) === null || _a === void 0 ? void 0 : _a.every(function (child) { return child.key !== grp.key; }); });
                            footerProps.group.children = __spreadArrays(footerProps.group.children, nonExistingChildren);
    ...
    return (React.createElement("div", null,
        React.createElement(GroupedList, { items: [], onRenderCell: null, groups: groups, groupProps: groupProps, onShouldVirtualize: function (p) { return false; }, "data-is-focusable": true, focusZoneProps: { direction: FocusZoneDirection.vertical, shouldEnterInnerZone: shouldEnterInnerZone } })));
}
